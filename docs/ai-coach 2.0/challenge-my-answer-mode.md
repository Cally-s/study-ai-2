...
> A challenge is offered only when permitted and cannot replace final writing, evade a review/source gate, or continue an active-assessment item.

> Challenge support can contribute to Level 8 verification/reflection only when independently permitted; it cannot extend the ladder or release a completed answer.

> Challenge My Answer requires its independent assignment permission and cannot turn draft feedback, brainstorming, or general explanation into an indirect final answer.

> Challenge My Answer runs only when Step 37 allows that task category and cannot become indirect feedback on a prohibited assessment or competition response.

> A completed quiz response may be challenged explicitly after its configured feedback point; no key is exposed early and the quiz does not silently change modes.

## Purpose
Challenge one material part of the student's actual answer so the student can defend, qualify, narrow, revise, or retain it. The mode never generates a hidden ideal answer or submission-ready rewrite.

## No automatic praise or disagreement
Generic praise and automatic opposition are prohibited. A supported feature may be acknowledged specifically. If no supported material challenge exists, say so within the reviewed scope.

## One challenge and student decision
Present one challenge, why it matters, one defend-or-revise prompt, an optional student-performed verification path, then wait. Another challenge requires explicit student action.

## Final position
The student states what changed or why the original position survived. All versions remain private and immutable.
