...
AIResponseRecord is the canonical trusted contract. The server owns identity, context, policy, sources, confidence, final-answer withholding, sharing, IDs, versions, timestamps, and validation state. READY records are immutable and tenant scoped. Corrections, source/assumption/policy changes, verification outcomes, and recalculation create a new linked record. Production persistence must retain policy/source snapshots, hashes, row versions, and access controls.
